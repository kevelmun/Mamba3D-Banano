...
class Mamba3D(nn.Module):
    ...
    def load_model_from_ckpt(self, bert_ckpt_path):
        if bert_ckpt_path is not None:
            ...
            # Reorganiza claves para compatibilidad
            for k in list(base_ckpt.keys()):
                if k.startswith('MAE_encoder'):
                    base_ckpt[k[len('MAE_encoder.'):]] = base_ckpt[k]
                    del base_ckpt[k]
            ...
            # Filtrar la classification head para que no se cargue
            for key in list(base_ckpt.keys()):
                if key.startswith("cls_head_finetune"):
                    print(f"Eliminando {key} del checkpoint para usar solo el backbone.")
                    del base_ckpt[key]
